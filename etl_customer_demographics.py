...
PathDB          = './data/postgres/customer_demographics/{}/{}.csv'
...
def extract_customer_demographics():
    FilePath = Path(PathDB.format(Data, 'customer_demographics'))
    FilePath.parent.mkdir(parents=True, exist_ok=True)
    
    Db = create_engine(Con_StringFrom)
    Connection = Db.connect()

    df = pd.read_sql_query("SELECT * FROM CUSTOMER_DEMOGRAPHICS", 
                            Connection) 
    df.to_csv(FilePath, header=True, index=False)
    
    return FilePath

def load_customer_demographics(ti):
    File = ti.xcom_pull(task_ids = 'extract_customer_demographics') 
    df = pd.read_csv(File)

    Db = create_engine(Con_StringTo)
    Connection = Db.connect()

    df.to_sql('customer_demographics', 
                con=Connection, 
                if_exists='append', 
                index=False)


with DAG('customer_demographics',
