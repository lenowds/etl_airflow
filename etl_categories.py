...
PathDB          = './data/postgres/categories/{}/{}.csv'
...
def extract_categories():
    FilePath = Path(PathDB.format(Data, 'categories'))
    FilePath.parent.mkdir(parents=True, exist_ok=True)
    
    Db = create_engine(Con_StringFrom)
    Connection = Db.connect()

    df = pd.read_sql_query("SELECT * FROM CATEGORIES", 
                            Connection) 
    df.to_csv(FilePath, header=True, index=False)
    
    return FilePath

def load_categories(ti):
    File = ti.xcom_pull(task_ids = 'extract_categories') 
    df = pd.read_csv(File)

    Db = create_engine(Con_StringTo)
    Connection = Db.connect()

    df.to_sql('categories', 
                con=Connection, 
                if_exists='append', 
                index=False)
